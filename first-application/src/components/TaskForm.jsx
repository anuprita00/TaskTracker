...
const TaskForm = () => {
  ...
  const selectTag = (tag) => {
    if(taskData.tags.some(item => item === tag)){ //checks if the tag exists in that array.
      ...
      setTaskData(prev => { //Then, we update the state using setTaskData(), removing the tag.
        return {...prev, tags:filterTags}
      })
    } else{ 
      setTaskData(prev => {  //updates the state with the new list of selected tags.
        return{...prev,
          tags:[...prev.tags, tag]} // Adds the new tag to the array.
      })
    }
  };
